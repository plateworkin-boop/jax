...
def _canonicalize_memory_space(
    memory_space: (
        pallas_core.MemorySpace | MemorySpace | tpu_core.CoreMemorySpace | None
    ),
    core_type: tpu_core.CoreType,
) -> MemorySpace | tpu_core.CoreMemorySpace:
  match memory_space, core_type:
    case pallas_core.MemorySpace.DEFAULT, tpu_core.CoreType.SC_VECTOR_SUBCORE:
      return MemorySpace.VMEM
    case pallas_core.MemorySpace.DEFAULT, tpu_core.CoreType.SC_SCALAR_SUBCORE:
      return MemorySpace.SMEM
    case tpu_core.MemorySpace() | tpu_core.CoreMemorySpace(), _:
      return memory_space
    case None, _:
      return MemorySpace.HBM
    case _:
      raise ValueError(
          f"Unsupported memory space: {memory_space!r} for kernel type"
          f" {core_type!r}"
      )
...
def _prepare_dma_refs(
  ...
  src_memory_space = _canonicalize_memory_space(
      src_aval.memory_space, core_type
  )
  dst_memory_space = _canonicalize_memory_space(
...
def _extract_indirect_offsets_from_indexer(
  ...
  if isinstance(offsets_aval, (state.AbstractRef, state.TransformedRef)):
    offsets_memory_space = _canonicalize_memory_space(
